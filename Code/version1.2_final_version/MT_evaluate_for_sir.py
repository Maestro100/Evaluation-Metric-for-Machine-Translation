...
def new_winF(): # new window definition
    ...
    '''
    width=newwin.winfo_width()
    height=newwin.winfo_height()
    x=(newwin.winfo_screenwidth() // 2) - (width // 2)
    y=(newwin.winfo_screenheight() // 2) - (height // 2)
    newwin.geometry('{}x{}+{}+{}'.format(width,height,x,y))
    '''

    fname_label = Label(newwin, text="Have a file with name readd ")
    #fname_entry = Entry(newwin)
    wb=load_workbook('readd.xlsx')
    BB=Button(newwin, text ="Calculate Scores",font="Times 15",command = lambda:calc(wb) )
