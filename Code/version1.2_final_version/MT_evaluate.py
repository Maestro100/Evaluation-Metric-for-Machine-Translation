...
def ShowAboutUs():
    tkMessageBox.showinfo("AKA",'Developed by - \n Arjya, Ayush and Kishan \n Hope this is put to good use' )

def CreateMenu():
    # create a pulldo/wn menu, and add it to the menu bar
    menubar = Menu(root)
    filemenu = Menu(menubar, tearoff=0)
    filemenu.add_command(label="Open", command=hello)
    filemenu.add_command(label="Save", command=hello)
    filemenu.add_separator()
    filemenu.add_command(label="Open new window", command=new_winF)
    filemenu.add_separator()
    ...
    helpmenu.add_command(label="About", command=ShowAboutUs)
